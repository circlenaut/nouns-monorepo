...
const FunctionCallSelectFunctionStep: React.FC<ProposalActionModalStepProps> = (
  props,
) => {
  ...
  const [value, setValue] = useState(
    state.amount ? state.amount.toString() : '',
  )
  ...
  useEffect(() => {
    const result = abi && Object.keys(abi.functions)?.[0]
    if (result) {
      setFunction(result)
    }
  }, [abi])
