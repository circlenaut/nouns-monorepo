...
const ChangeDelegatePannel: React.FC<ChangeDelegatePannelProps> = props => {
  ...
  const availableVotes = useNounTokenBalance(contractAddresses, activeAccount ?? '') ?? 0;
  const { send: delegateVotes, state: delegateState } = useDelegateVotes(contractAddresses);
  ...
  const accountVotes = useAccountVotes(contractAddresses, activeAccount ?? '') ?? 0;
  ...
  const primaryButton = usePickByState(
    ...
    [
      <NavBarButton key={activeAccount}
        buttonText={
