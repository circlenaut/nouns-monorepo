...
const useOnDisplayAuction = (): Auction | undefined => {
  const lastAuctionNounId = useAppSelector(
    (state) => state.auction.activeAuction?.nounId,
  )
  ...
  const reduxSafeAuction: Auction | undefined = pastAuctions.find(
    (auction: AuctionState) => {
      const nounId =
        auction.activeAuction && BigNumber.from(auction.activeAuction.nounId)
...
export const useAuctionBids = (auctionNounId: BigNumber): Bid[] | undefined => {
  ...
  if (lastAuctionNounId === auctionNounId.toNumber()) {
    ...
    const bidEvents: BidEvent[] | undefined = pastAuctions.find(
      (auction: AuctionState) => {
        const nounId =
          auction.activeAuction && BigNumber.from(auction.activeAuction.nounId)
