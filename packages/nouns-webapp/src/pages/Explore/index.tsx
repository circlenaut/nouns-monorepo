...
const ExplorePage: React.FC = () => {
  ...
  const currentAuction: IAuction | undefined = useAppSelector(
    (state) => state.auction.activeAuction,
  )
  const nounCount = currentAuction
    ? BigNumber.from(currentAuction?.nounId).toNumber() + 1
    : -1
  ...
  useEffect(() => {
    if (nounCount >= 0) {
      ...
      const id = BigNumber.from(currentAuction?.nounId).toNumber()
      !isMobile && setSelectedNoun(id)
      !isMobile && setActiveNoun(id)
    }
  }, [nounCount])
  ...
  useEffect(() => {
    ...
    if (!isNounHoverDisabled) {
      window.addEventListener(
        'mousemove',
        (event) => {
          const noun = document.querySelector('.noun')
          if (noun) {
            const { x, y, width, height } = noun.getBoundingClientRect()
  ...
  const nounsReverseList = nounsList
    .filter((n) => n.id !== undefined && n.id !== null && n.id >= 0)
    .reverse()[activeNoun]
    ...
    <div className={classes.exploreWrap} ref={containerRef}>
      <div className={classes.contentWrap}>
        ...
        <AnimatePresence initial={false}>
          {isSidebarVisible && nounsReverseList && (
            <ExploreNounDetail
              handleCloseDetail={() => handleCloseDetail()}
              handleNounNavigation={handleNounNavigation}
              noun={nounsReverseList}
              activeNounId={activeNoun}
              nounCount={nounCount}
              selectedNoun={selectedNoun}
